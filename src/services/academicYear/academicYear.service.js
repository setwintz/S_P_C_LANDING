...
const create = async (data) => {
    try {
        ...
        const response = await api.post(`${import.meta.env.VITE_API_URL}/api/v1/client/academic/academicYear/create/academicYear`, { ...data },
            {
                headers: {
...
const getList = async (currentPage, rowsPerPage, text, tenantId, enterpriseId, selectedStatus, selectedUnitId) => {
    try {
        const authToken = localStorage.getItem("SAAS_ERP1_CLIENT_TACCESS_TOKEN");
        const response = await api.get('/api/v1/client/academic/academicYear/list/academicYear', {
            headers: {
                Authorization: `Bearer ${authToken}`,
            },
            params: {
