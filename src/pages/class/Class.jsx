...
function Class({ noFade = false }) {
    ...
    const [showClassFilterModel, setShowClassFilterModel] = useState(false); // Fixed name
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            let response;
            if (editId) {
                response = await classService.update({
                    classId: editId, // Fixed: was mediumId
                ...
                response = await classService.create(payload);
            }
    ...
    const fetchList = useCallback(async (page, perPage, keyword) => {
        try {
            ...
            const res = await classService.getList(page, perPage, keyword, unitId, entUuid, selectedStatus, selectedUnitId);
    ...
    const toggleStatus = async (currentStatus, id) => {
        ...
        try {
            ...
            const res = await classService.activeInactive({
            ...
            setUpdatedData(res.data?.data || []);
            setTotalData(res.data?.total || 0);
            toast.success(`Class ${currentStatus ? 'deactivated' : 'activated'}`);
        } catch {
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            ...
            try {
                ...
                const res = await classService.softDelete({
                    id,
                    page: currentPage,
                    perPage: rowsPerPage,
                    keyword: searchText,
    ...
    const handleRestore = async (id) => {
        ...
        if (result.isConfirmed) {
            ...
            try {
                ...
                const res = await classService.restore({
                    id,
                    page: currentPage,
                    perPage: rowsPerPage,
                    keyword: searchText,
    ...
    const columns = useMemo(() => {
        const base = [
            ...
            },
            {
                ...
                width: '250px',
                render: (value) => (
                    <span className="text-gray-800 dark:text-gray-300 text-sm">{value || '-'}</span>
                ),
            },
            {
                header: 'Status',
    ...
    return (
        <div className="w-[100%] mx-auto mt-4 mb-10   lg:px-2 px-0">
            <CustomTable
                ...
                totalData={totalData}
                noData={<NoClass />}
                filterToggleValue={showClassFilterModel}
                filterToggleFunction={setShowClassFilterModel}
                            ...
                            <Dialog.Panel className="w-[100%] max-w-lg bg-white dark:bg-cardBgDark rounded-lg shadow-xl flex flex-col max-h-[80vh]">
                                <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-upperCardBgDark">
                                    <h2 className="text-xl font-bold text-gray-900 dark:text-white">
                                        {isViewMode ? 'View Class' : editId ? 'Edit Class' : 'Create Class'}
                                    </h2>
                                    <button
                                        onClick={closeForm}
                                ...
                                <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            placeholder="e.g. 10th, 12th, Nursery"
                                            required
                                            disabled={isViewMode}
                                        />
            ...
            <ClassFilters
                ...
                units={units}
                showFilter={showClassFilterModel}
                setShowFilter={setShowClassFilterModel}
            />
