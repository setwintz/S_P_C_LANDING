...
function ListOrganization({ noFade }) {
    ...
    const columns = [
        ...
        {
            key: 'unit_head', header: 'Client Name', with: "auto",
            render: (value, row) => {
        ...
        {
            key: 'unit_head', header: 'Client Email', with: "auto",
            render: (value, row) => {
                const name = value ? value?.email: ""
                return (
                    <span className={` whitespace-nowrap  text-black dark:text-white  rounded-md`} >
                        {name ? name : "Type not found"}
                    </span>
         ...
         {
            key: 'unit_head', header: 'Client Phone', with: "auto",
            render: (value, row) => {
                const name = value ? value?.phone: ""
                return (
                    <span className={` whitespace-nowrap  text-black dark:text-white  rounded-md`} >
                        {name ? name : "Type not found"}
                    </span>
        ...
        {
            key: 'name', header: 'Organization Name', with: "auto",
            render: (value, row) => {
                const name = value 
                return (
                    <span className={`${name ? "bg-custom-gradient-soft-blue" : "bg-custom-gradient-red"} text-[.80rem] whitespace-nowrap font-bold text-white px-2 py-2 rounded-full`} >
        ...
        },
        {
            ...
            render: (value, row) => (
                <div className='flex gap-3'>
                    ...
                    >
                        <button
                            className='bg-custom-gradient-green p-2 shadow-2xl rounded-full'
                        ...
                        row?.deletedAt ?
                            ...
                            >
                                <button
                                    ...
                                    className='bg-custom-gradient-red p-2 shadow-xl rounded-full'
                                >
    ...
    async function getUnits(currentPage, rowsPerPage, text) {
        try {
            const response = await clientService.getUnits(currentPage, rowsPerPage, text);
            return response
        } catch (error) {
            throw error
        }
     ...
     async function getEnterprises(currentPage, rowsPerPage, text) {
        try {
            const response = await clientService.getEnterprises(currentPage, rowsPerPage, text);
            return response
        } catch (error) {
            throw error
        }
    ...
    return (
        <div className="max-w-7xl mx-auto mt-8 mb-10 px-2 sm:px-4 lg:px-8">
            ...
            <h2 className="md:text-2xl text-1xl font-semibold text-formHeadingLight dark:text-orangeBorder mb-2 text-start">{`All Organizations`}</h2>
            <div className="h-[1.8px] bg-formHeadingLight dark:bg-orangeBorder mb-4"></div>
            <CustomTable
                columns={columns}
                fetchData={getUnits}
                headerBackground={isDark ? "#daa366" : "#2196f3"}
                headerTextColor="#fff"
